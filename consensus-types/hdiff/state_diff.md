...
The current document describes the implementation details and the design of hierarchical state diffs on Sila. They follow the same design as [Lighthouse](https://github.com/dapplion/tree-states-review-guide/blob/main/persisted_hdiff.md) which in turn is an implementation of A. Nashatyrev's [design](https://hackmd.io/G82DNSdvR5Osw2kg565lBA).

Incremental state diffs can be used both for databases and memory representations of states. This document focuses on the state diffs necessary for the first usage. Sila already handles memory deduplication of states with multi value slices, thus a diff mechanism would result in less impact.
...
Summarizing, the database has no changes in the exposed API, minimizing changes in the overal Sila implementation, while the database internally changes the functions `State` and `SaveState` to use the `consensus-types/hdiff` package. This makes the serialization package fairly contained and only accessible from within the database package.
...
type stateDiff struct {
	...
	sila1Data                    *silapb.Sila1Data
	sila1DataVotes               []*silapb.Sila1Data
	sila1DepositIndex            uint64
	...
	pendingDepositDiff             []*silapb.PendingDeposit
	pendingPartialWithdrawalsDiff  []*silapb.PendingPartialWithdrawal
	pendingConsolidationsDiffs     []*silapb.PendingConsolidation
...
The Sila1 Data diff object is treated exactly like the fork object.

##### Sila1 Data Votes

The `stateDiff`  structure has two fields related to Sila1 data votes. The boolean entry `sila1VotesAppend` and a slice `sila1DataVotes`. The boolean indicates if the slice is to be *appended* to the source target or if the sila1 data vote slice needs to be completely replaced with the slice in the diff.

Deserialization then goes as follows, if the first byte is `nilMarker` then `sila1VotesAppend`  is set to `True`, and `False`  otherwise. The following 8 bytes contain a `uint64` serialization of the length of the slice. The remaining bytes contain the serialized slice.
...
**Latest Sila Payload Bid.** Always non-nil for valid Gloas states. Serialized as 8 bytes for the SSZ size followed by the SSZ bytes (variable size due to `BlobKzgCommitments`). The bid carries 12 sub-fields including `SilaRequestsRoot`. Always overrides. The diff computation errors if the target bid is nil.
