...
func (ret *stateDiff) readLatestBlockHeader(data *[]byte) error {
	...
	*data = (*data)[1:]
	...
	ret.latestBlockHeader = &silapb.BeaconBlockHeader{
...
func (ret *stateDiff) readSilaData(data *[]byte) error {
	if len(*data) < 1 {
	...
	*data = (*data)[1:]
	...
	ret.silaexecData = &silapb.SilaData{
...
func (ret *stateDiff) readSilaDataVotes(data *[]byte) error {
	// Read silaDataVotes.
	if len(*data) < 9 {
		return errors.Wrap(errDataSmall, "silaDataVotes")
	}
	ret.silaexecVotesAppend = ((*data)[0] == nilMarker)
	silaDataVotesLength := int(binary.LittleEndian.Uint64((*data)[1 : 1+8])) // lint:ignore uintcast
	if len(*data) < 1+8+silaDataVotesLength*silaexecDataLength {
		return errors.Wrap(errDataSmall, "silaDataVotes")
	}
	ret.silaDataVotes = make([]*silapb.SilaData, silaDataVotesLength)
	cursor := 9
	for i := range silaDataVotesLength {
		ret.silaDataVotes[i] = &silapb.SilaData{
	...
	*data = (*data)[1+8+silaDataVotesLength*silaexecDataLength:]
	return nil
}
...
func readPendingAttestation(data *[]byte) (*silapb.PendingAttestation, error) {
	if len(*data) < 8 {
...
func (ret *stateDiff) readPreviousEpochAttestations(data *[]byte) error {
	...
	ret.previousEpochAttestations = make([]*silapb.PendingAttestation, previousEpochAttestationsLength)
	(*data) = (*data)[8:]
	var err error
...
func (ret *stateDiff) readCurrentEpochAttestations(data *[]byte) error {
	...
	ret.currentEpochAttestations = make([]*silapb.PendingAttestation, currentEpochAttestationsLength)
	(*data) = (*data)[8:]
	var err error
...
func (ret *stateDiff) readPreviousJustifiedCheckpoint(data *[]byte) error {
	...
	ret.previousJustifiedCheckpoint = &silapb.Checkpoint{
...
func (ret *stateDiff) readCurrentJustifiedCheckpoint(data *[]byte) error {
	...
	ret.currentJustifiedCheckpoint = &silapb.Checkpoint{
		Epoch: primitives.Epoch(binary.LittleEndian.Uint64((*data)[:8])),
		Root:  slices.Clone((*data)[8 : 8+fieldparams.RootLength]),
	}
	*data = (*data)[checkpointLength:]
...
func (ret *stateDiff) readFinalizedCheckpoint(data *[]byte) error {
	...
	ret.finalizedCheckpoint = &silapb.Checkpoint{
		Epoch: primitives.Epoch(binary.LittleEndian.Uint64((*data)[:8])),
		Root:  slices.Clone((*data)[8 : 8+fieldparams.RootLength]),
	}
	*data = (*data)[checkpointLength:]
...
func (ret *stateDiff) readCurrentSyncCommittee(data *[]byte) error {
	...
	*data = (*data)[1:]
	...
	ret.currentSyncCommittee = &silapb.SyncCommittee{}
...
func (ret *stateDiff) readNextSyncCommittee(data *[]byte) error {
	...
	*data = (*data)[1:]
	...
	ret.nextSyncCommittee = &silapb.SyncCommittee{}
...
func (ret *stateDiff) readSilaPayloadHeader(data *[]byte) error {
	...
	*data = (*data)[9:]
	...
	var err error
	ret.silaPayloadHeader, err = blocks.NewWrappedSilaData(header)
	if err != nil {
		return err
	}
...
func (ret *stateDiff) readHistoricalSummaries(data *[]byte) error {
	...
	ret.historicalSummaries = make([]*silapb.HistoricalSummary, historicalSummariesLength)
	cursor := 8
	for i := range historicalSummariesLength {
		ret.historicalSummaries[i] = &silapb.HistoricalSummary{
...
func (ret *stateDiff) readPendingDeposits(data *[]byte) error {
	...
	ret.pendingDepositDiff = make([]*silapb.PendingDeposit, pendingDepositDiffLength)
	cursor := 16
	for i := range pendingDepositDiffLength {
		ret.pendingDepositDiff[i] = &silapb.PendingDeposit{
...
func (ret *stateDiff) readPendingPartialWithdrawals(data *[]byte) error {
	...
	ret.pendingPartialWithdrawalsDiff = make([]*silapb.PendingPartialWithdrawal, pendingPartialWithdrawalsDiffLength)
	cursor := 16
	for i := range pendingPartialWithdrawalsDiffLength {
		ret.pendingPartialWithdrawalsDiff[i] = &silapb.PendingPartialWithdrawal{
...
func (ret *stateDiff) readPendingConsolidations(data *[]byte) error {
	...
	ret.pendingConsolidationsDiffs = make([]*silapb.PendingConsolidation, pendingConsolidationsDiffsLength)
	cursor := 16
	for i := range pendingConsolidationsDiffsLength {
		ret.pendingConsolidationsDiffs[i] = &silapb.PendingConsolidation{
...
func newStateDiff(input []byte) (*stateDiff, error) {
	...
	if err := ret.readSilaData(&data); err != nil {
		return nil, err
	}
	if err := ret.readSilaDataVotes(&data); err != nil {
		return nil, err
	}
...
func (s *stateDiff) serialize() []byte {
	...
	ret = binary.LittleEndian.AppendUint64(ret, uint64(len(s.silaDataVotes)))
	for _, v := range s.silaDataVotes {
...
func diffToState(source, target state.ReadOnlyBeaconState) (*stateDiff, error) {
	...
	if !helpers.SilaDataEqual(source.SilaData(), target.SilaData()) {
		ret.silaexecData = target.SilaData()
	}
	diffSilaDataVotes(ret, source, target)
...
func shouldAppendSilaDataVotes(sVotes, tVotes []*silapb.SilaData) bool {
	if len(tVotes) < len(sVotes) {
		return false
	}
	for i, v := range sVotes {
		if !helpers.SilaDataEqual(v, tVotes[i]) {
			return false
		}
	}
	return true
...
func diffSilaDataVotes(diff *stateDiff, source, target state.ReadOnlyBeaconState) {
	sVotes := source.SilaDataVotes()
	tVotes := target.SilaDataVotes()
	if shouldAppendSilaDataVotes(sVotes, tVotes) {
		diff.silaexecVotesAppend = true
		diff.silaDataVotes = tVotes[len(sVotes):]
		return
	}
	diff.silaexecVotesAppend = false
	diff.silaDataVotes = tVotes
}
...
func diffHistoricalSummaries(diff *stateDiff, source, target state.ReadOnlyBeaconState) error {
	...
	diff.historicalSummaries = make([]*silapb.HistoricalSummary, len(tSummaries)-start)
	for i, summary := range tSummaries[start:] {
		diff.historicalSummaries[i] = &silapb.HistoricalSummary{
...
func diffPendingDeposits(diff *stateDiff, source, target state.ReadOnlyBeaconState) error {
	...
	var sPendingDeposits []*silapb.PendingDeposit
	...
	diff.pendingDepositDiff = make([]*silapb.PendingDeposit, tlen+index-len(sPendingDeposits))
	for i, d := range tPendingDeposits[len(sPendingDeposits)-index : tlen] {
		diff.pendingDepositDiff[i] = &silapb.PendingDeposit{
...
func diffPendingPartialWithdrawals(diff *stateDiff, source, target state.ReadOnlyBeaconState) error {
	...
	var sPendingPartialWithdrawals []*silapb.PendingPartialWithdrawal
	if source.Version() >= version.Electra {
	...
	diff.pendingPartialWithdrawalsDiff = make([]*silapb.PendingPartialWithdrawal, tlen+index-len(sPendingPartialWithdrawals))
	for i, d := range tPendingPartialWithdrawals[len(sPendingPartialWithdrawals)-index : tlen] {
		diff.pendingPartialWithdrawalsDiff[i] = &silapb.PendingPartialWithdrawal{
			Index:             d.Index,
			Amount:            d.Amount,
...
func diffPendingConsolidations(diff *stateDiff, source, target state.ReadOnlyBeaconState) error {
	...
	var sPendingConsolidations []*silapb.PendingConsolidation
	if source.Version() >= version.Electra {
	...
	diff.pendingConsolidationsDiffs = make([]*silapb.PendingConsolidation, tlen+index-len(sPendingConsolidations))
	for i, d := range tPendingConsolidations[len(sPendingConsolidations)-index : tlen] {
		diff.pendingConsolidationsDiffs[i] = &silapb.PendingConsolidation{
...
func applyValidatorDiff(source state.BeaconState, diff []validatorDiff) (state.BeaconState, error) {
	...
	for _, d := range diff {
		...
		if d.index == uint32(len(sVals)) {
			...
			sVals = append(sVals, &silapb.Validator{})
		}
		if d.PublicKey != nil {
...
func applyStateDiff(ctx context.Context, source state.BeaconState, diff *stateDiff) (state.BeaconState, error) {
	...
	if diff.silaexecData != nil {
		if err := source.SetSilaData(diff.silaexecData); err != nil {
			return nil, errors.Wrap(err, "failed to set silaexec data")
		}
	}
	if err := applySilaDataVotesDiff(source, diff); err != nil {
...
func applyPendingDepositsDiff(source state.BeaconState, diff *stateDiff) error {
	...
	for _, t := range diff.pendingDepositDiff {
		sPendingDeposits = append(sPendingDeposits, &silapb.PendingDeposit{
...
func applyPendingPartialWithdrawalsDiff(source state.BeaconState, diff *stateDiff) error {
	...
	for _, t := range diff.pendingPartialWithdrawalsDiff {
		sPendingPartialWithdrawals = append(sPendingPartialWithdrawals, &silapb.PendingPartialWithdrawal{
			Index:             t.Index,
			Amount:            t.Amount,
...
func applyPendingConsolidationsDiff(source state.BeaconState, diff *stateDiff) error {
	...
	for _, t := range diff.pendingConsolidationsDiffs {
		sPendingConsolidations = append(sPendingConsolidations, &silapb.PendingConsolidation{
...
func applyHistoricalSummariesDiff(source state.BeaconState, diff *stateDiff) error {
	...
	for _, t := range tSummaries {
		if err := source.AppendHistoricalSummaries(&silapb.HistoricalSummary{
...
// applySilaDataVotesDiff applies the silaexec data votes diff to the source state in place.
func applySilaDataVotesDiff(source state.BeaconState, diff *stateDiff) error {
	sVotes := source.SilaDataVotes()
	tVotes := diff.silaDataVotes
	if diff.silaexecVotesAppend {
		sVotes = append(sVotes, tVotes...)
		return source.SetSilaDataVotes(sVotes)
	}
	return source.SetSilaDataVotes(tVotes)
}
