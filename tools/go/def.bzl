...
def _go_test_transition_impl(settings, attr):
    ...
    elif attr.sila_network == "mainnet":  # Default / optional
...
def _go_test_transition_rule(**kwargs):
    """A wrapper around go_test to add an sila_network attribute and incoming edge transition to support compile time configuration"""
    kwargs = dict(kwargs)
    attrs = dict(kwargs["attrs"])
    attrs.update({
        "sila_network": attr.string(values = ["mainnet", "minimal"]),
    })
    kwargs["attrs"] = attrs
