...
// testSignedEnvelope creates a SignedSilaPayloadEnvelope that is valid for SSZ
// encoding. All fixed-size byte fields are zero-filled to the required length.
func testSignedEnvelope(slot primitives.Slot, beaconBlockRoot []byte) *pb.SignedSilaPayloadEnvelope {
	root := make([]byte, 32)
	copy(root, beaconBlockRoot)
	return &pb.SignedSilaPayloadEnvelope{
		Message: &pb.SilaPayloadEnvelope{
			Payload: &engpb.SilaPayloadGloas{
...
func TestValidateEnvelopesByRange(t *testing.T) {
	...
	tests := []struct {
		name       string
		req        *pb.SilaPayloadEnvelopesByRangeRequest
	...
	}{
		{
			...
			req:       &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: 100, Count: 0},
			current:   200,
			expectErr: true,
		},
		{
			name:      "overflow start plus count minus one",
			req:       &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: maxUint - 5, Count: 10},
			current:   maxUint,
			expectErr: true,
		},
		{
			name:       "start after current returns empty rangeParams",
			req:        &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: 500, Count: 10},
			current:    400,
			expectedRP: &rangeParams{start: 400, end: 400, size: 0},
		},
		{
			name:       "normal range within limits",
			req:        &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: gloasStart + 10, Count: 20},
		...
		},
		{
			...
			req:        &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: gloasStart - 10, Count: 50},
			current:    gloasStart + 200,
			expectedRP: &rangeParams{start: gloasStart, end: gloasStart + 39, size: 50},
		},
		{
			name:       "end clamped to current slot",
			req:        &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: gloasStart, Count: 500},
		...
		},
		{
			...
			req:        &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: gloasStart, Count: 1000},
...
func TestValidateSilaPayloadEnvelopeByRangeResponseRejectsMalformedEnvelope(t *testing.T) {
	req := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: 5, Count: 1}

	t.Run("nil message", func(t *testing.T) {
		env := testSignedEnvelope(5, make([]byte, 32))
		env.Message = nil

		_, _, err := validateSilaPayloadEnvelopeByRangeResponse(env, req, 0, nil, false)
		require.NotNil(t, err)
		assert.ErrorContains(t, "invalid sila payload envelope", err)
	})
...
// ---------------------------------------------------------------------------
// silaPayloadEnvelopesByRangeRPCHandler (server handler)
// ---------------------------------------------------------------------------

func TestSilaPayloadEnvelopesByRangeRPCHandler(t *testing.T) {
	params.SetupTestConfigCleanup(t)
	cfg := params.BeaconConfig().Copy()
	...
	topicFmt := fmt.Sprintf("%s/ssz_snappy", p2p.RPCSilaPayloadEnvelopesByRangeTopicV1)
	...
	t.Run("wrong message type", func(t *testing.T) {
		...
		herr := svc2.silaPayloadEnvelopesByRangeRPCHandler(ctx, "not-a-request", stream)
	...
	t.Run("invalid request count=0", func(t *testing.T) {
		...
		msg := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: 10, Count: 0}
		handlerErr := svc.silaPayloadEnvelopesByRangeRPCHandler(ctx, msg, stream)
	...
	t.Run("nominal sends chunks for saved envelopes", func(t *testing.T) {
		...
		for i, sl := range blockSlots {
			...
			copy(blk.Block.Body.SignedSilaPayloadBid.Message.ParentBlockHash, parentRoot[:])
		...
		remoteP2P.BHost.SetStreamHandler(protocolID, func(stream network.Stream) {
			defer wg.Done()
			for {
				env, readErr := readChunkedSilaPayloadEnvelope(stream, remoteP2P.Encoding(), ctxMap)
		...
		msg := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: 5, Count: 31}
		handlerErr := svc.silaPayloadEnvelopesByRangeRPCHandler(ctx, msg, stream)
...
// ---------------------------------------------------------------------------
// SendSilaPayloadEnvelopesByRangeRequest (client)
// ---------------------------------------------------------------------------

func TestSendSilaPayloadEnvelopesByRangeRequest(t *testing.T) {
	params.SetupTestConfigCleanup(t)
	cfg := params.BeaconConfig().Copy()
	cfg.GloasForkEpoch = 0
	cfg.MaxRequestPayloads = 128
	...
	topic := fmt.Sprintf("%s/ssz_snappy", p2p.RPCSilaPayloadEnvelopesByRangeTopicV1)
	ctx := t.Context()
	...
	t.Run("receives envelopes from remote peer", func(t *testing.T) {
		...
		req := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: startSlot, Count: count}
		envelopes, recvErr := SendSilaPayloadEnvelopesByRangeRequest(ctx, clock, p1, p2.PeerID(), ctxMap, req)
	...
	t.Run("empty response from remote peer", func(t *testing.T) {
		...
		req := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: 5, Count: 10}
		envelopes, recvErr := SendSilaPayloadEnvelopesByRangeRequest(ctx, clock, p1, p2.PeerID(), ctxMap, req)
		require.NoError(t, recvErr)
	...
	t.Run("slot out of requested range returns error", func(t *testing.T) {
		...
		req := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: startSlot, Count: count}
		_, recvErr := SendSilaPayloadEnvelopesByRangeRequest(ctx, clock, p1, p2.PeerID(), ctxMap, req)
	...
	t.Run("slots not monotonically increasing returns error", func(t *testing.T) {
		...
		p2.SetStreamHandler(topic, func(stream network.Stream) {
			...
			assert.NoError(t, WriteSilaPayloadEnvelopeChunk(stream, p2.Encoding(), env1))
			assert.NoError(t, WriteSilaPayloadEnvelopeChunk(stream, p2.Encoding(), env2))
		})

		req := &pb.SilaPayloadEnvelopesByRangeRequest{StartSlot: startSlot, Count: 10}
		_, recvErr := SendSilaPayloadEnvelopesByRangeRequest(ctx, clock, p1, p2.PeerID(), ctxMap, req)
		require.NotNil(t, recvErr)
	...
	t.Run("peer exceeds requested count returns error", func(t *testing.T) {
		...
		_, recvErr := SendSilaPayloadEnvelopesByRangeRequest(ctx, clock, p1, p2.PeerID(), ctxMap, req)
		require.NotNil(t, recvErr)
		assert.ErrorContains(t, "more sila payload envelopes than requested", recvErr)
	})
}
