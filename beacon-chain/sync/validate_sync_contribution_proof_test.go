...
func TestService_ValidateSyncContributionAndProof(t *testing.T) {
	...
	type args struct {
		pid   peer.ID
		msg   *silapb.SignedContributionAndProof
		topic string
	}
	tests := []struct {
		name     string
		svcopts  []Option
		setupSvc func(s *Service, msg *silapb.SignedContributionAndProof) (*Service, *startup.Clock)
		clock    *startup.Clock
		args     args
...
func fillUpBlocksAndState(ctx context.Context, t *testing.T, beaconDB db.Database) ([32]byte, []bls.SecretKey) {
	...
	for i := primitives.Slot(1); i <= params.BeaconConfig().SlotsPerEpoch; i++ {
		...
		assert.NoError(t, beaconDB.SaveStateSummary(ctx, &silapb.StateSummary{Slot: i, Root: r[:]}))
...
func syncSelectionProofSigningRoot(st state.BeaconState, slot primitives.Slot, comIdx primitives.CommitteeIndex) ([32]byte, error) {
	...
	selectionData := &silapb.SyncAggregatorSelectionData{Slot: slot, SubcommitteeIndex: uint64(comIdx)}
...
func TestService_setSyncContributionIndexSlotSeen(t *testing.T) {
	...
	b0.SetBitAt(0, true)
	has, err := s.hasSeenSyncContributionBits(&silapb.SyncCommitteeContribution{
		AggregationBits: b0,
	})
	require.NoError(t, err)
	...
	require.NoError(t, s.setSyncContributionBits(&silapb.SyncCommitteeContribution{
		AggregationBits: b0,
	}))
	has, err = s.hasSeenSyncContributionBits(&silapb.SyncCommitteeContribution{
		AggregationBits: b0,
