...
func (c *Cache) allDeposits(untilBlk *big.Int) []*silapb.Deposit {
	...
	for _, ctnr := range c.deposits {
		cBlk := big.NewInt(0).SetUint64(ctnr.SilaexecBlockHeight)
...
func (c *Cache) DepositByPubkey(ctx context.Context, pubKey []byte) (*silapb.Deposit, *big.Int) {
	...
	blockNum = big.NewInt(int64(deps[0].SilaexecBlockHeight))
	return deposit, blockNum
}
...
func (c *Cache) DepositsNumberAndRootAtHeight(ctx context.Context, blockHeight *big.Int) (uint64, [32]byte) {
	...
	heightIdx := sort.Search(len(c.deposits), func(i int) bool {
		dBlkHeight := big.NewInt(0).SetUint64(c.deposits[i].SilaexecBlockHeight)
		return dBlkHeight.Cmp(blockHeight) > 0
	})
	// send the deposit root of the empty trie, if silaexecfollow distance is greater than the time of the earliest
	// deposit.
	if heightIdx == 0 {
		return 0, [32]byte{}
	}
...
func (c *Cache) NonFinalizedDeposits(ctx context.Context, lastFinalizedIndex int64, untilBlk *big.Int) []*silapb.Deposit {
	...
	for _, d := range c.deposits {
		if (d.Index > lastFinalizedIndex) && (untilBlk == nil || untilBlk.Uint64() >= d.SilaexecBlockHeight) {
...
func (c *Cache) InsertPendingDeposit(ctx context.Context, d *silapb.Deposit, blockNum uint64, index int64, depositRoot [32]byte) {
	...
	c.pendingDeposits = append(c.pendingDeposits,
		&silapb.DepositContainer{Deposit: d, SilaexecBlockHeight: blockNum, Index: index, DepositRoot: depositRoot[:]})
...
func (c *Cache) PendingContainers(ctx context.Context, untilBlk *big.Int) []*silapb.DepositContainer {
	...
	for _, ctnr := range c.pendingDeposits {
		if untilBlk == nil || untilBlk.Uint64() >= ctnr.SilaexecBlockHeight {
