...
func TestEnvelopeVerifier_VerifySlotMatchesBlock(t *testing.T) {
	root := bytesutil.ToBytes32(bytes.Repeat([]byte{0xAA}, 32))
	blockHash := bytesutil.ToBytes32(bytes.Repeat([]byte{0xBB}, 32))
	env := testSignedSilaPayloadEnvelope(t, 2, 1, root, blockHash)
	wrapped, err := blocks.WrappedROSignedSilaPayloadEnvelope(env)
	require.NoError(t, err)
...
func TestEnvelopeVerifier_VerifyBuilderValid(t *testing.T) {
	...
	badBid := testSilaPayloadBid(t, 1, 2, blockHash)
	...
	okBid := testSilaPayloadBid(t, 1, 1, blockHash)
	verifier = &EnvelopeVerifier{results: newResults(RequireBuilderValid), e: wrapped}
...
func TestEnvelopeVerifier_VerifyPayloadHash(t *testing.T) {
	...
	badBid := testSilaPayloadBid(t, 1, 1, badHash)
...
func TestEnvelopeVerifier_VerifySignature_Builder(t *testing.T) {
	...
	root := bytesutil.ToBytes32(bytes.Repeat([]byte{0xAA}, 32))
	blockHash := bytesutil.ToBytes32(bytes.Repeat([]byte{0xBB}, 32))
	env := testSignedSilaPayloadEnvelope(t, slot, 0, root, blockHash)
...
func TestEnvelopeVerifier_VerifySignature_SelfBuild(t *testing.T) {
	...
	root := bytesutil.ToBytes32(bytes.Repeat([]byte{0xAA}, 32))
	blockHash := bytesutil.ToBytes32(bytes.Repeat([]byte{0xBB}, 32))
	env := testSignedSilaPayloadEnvelope(t, slot, params.BeaconConfig().BuilderIndexSelfBuild, root, blockHash)
...
func testSilaPayloadBid(t *testing.T, slot primitives.Slot, builderIdx primitives.BuilderIndex, blockHash [32]byte) interfaces.ROSilaPayloadBid {
	t.Helper()

	signed := util.GenerateTestSignedSilaPayloadBid(slot)
...
func signEnvelope(t *testing.T, sk bls.SecretKey, env *silapb.SilaPayloadEnvelope, fork *silapb.Fork, genesisRoot []byte, slot primitives.Slot) [96]byte {
	t.Helper()
