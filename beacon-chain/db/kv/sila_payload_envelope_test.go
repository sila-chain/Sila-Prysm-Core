...
func testEnvelope(t *testing.T) *silapb.SignedSilaPayloadEnvelope {
	t.Helper()
	return &silapb.SignedSilaPayloadEnvelope{
		Message: &silapb.SilaPayloadEnvelope{
			Payload: &silaenginev1.SilaPayloadGloas{
				...
				Withdrawals:   []*silaenginev1.Withdrawal{{Index: 1, ValidatorIndex: 2, Address: bytesutil.PadTo([]byte("addr"), 20), Amount: 100}},
				BlobGasUsed:   131072,
				ExcessBlobGas: 0,
				SlotNumber:    99,
			},
...
func TestStore_SaveAndRetrieveSilaPayloadEnvelope(t *testing.T) {
	db := setupDB(t)
	...
	assert.Equal(t, false, db.HasSilaPayloadEnvelope(ctx, blockRoot))

	// Save (always blinds internally).
	require.NoError(t, db.SaveSilaPayloadEnvelope(ctx, env))

	// Should exist now.
	assert.Equal(t, true, db.HasSilaPayloadEnvelope(ctx, blockRoot))

	// Load and verify it's blinded.
	loaded, err := db.SilaPayloadEnvelope(ctx, blockRoot)
...
func TestStore_DeleteSilaPayloadEnvelope(t *testing.T) {
	db := setupDB(t)
	ctx := context.Background()
	env := testEnvelope(t)
	blockRoot := bytesutil.ToBytes32(env.Message.BeaconBlockRoot)
	...
	require.NoError(t, db.DeleteSilaPayloadEnvelope(ctx, blockRoot))
	assert.Equal(t, false, db.HasSilaPayloadEnvelope(ctx, blockRoot))
}

func TestStore_SilaPayloadEnvelope_NotFound(t *testing.T) {
	db := setupDB(t)
	ctx := context.Background()
	nonExistent := bytesutil.ToBytes32([]byte("nonexistent"))

	_, err := db.SilaPayloadEnvelope(ctx, nonExistent)
	require.ErrorContains(t, "not found", err)
}

func TestStore_SaveSilaPayloadEnvelope_NilRejected(t *testing.T) {
	db := setupDB(t)
	ctx := context.Background()

	err := db.SaveSilaPayloadEnvelope(ctx, nil)
	require.ErrorContains(t, "nil", err)
}

func TestStore_SilaPayloadEnvelopeByBlockHash(t *testing.T) {
	db := setupDB(t)
	ctx := context.Background()
	env := testEnvelope(t)
	...
	loaded, err := db.SilaPayloadEnvelopeByBlockHash(ctx, blockHash)
	require.NoError(t, err)
	assert.Equal(t, primitives.Slot(env.Message.Payload.SlotNumber), loaded.Message.Slot)
	assert.DeepEqual(t, env.Message.Payload.BlockHash, loaded.Message.BlockHash)
	assert.Equal(t, true, bytes.Equal(env.Message.Payload.ParentHash, loaded.Message.ParentBlockHash))
...
func TestStore_SilaPayloadEnvelopeByBlockHash_NotFound(t *testing.T) {
	db := setupDB(t)
	ctx := context.Background()
	nonExistent := bytesutil.ToBytes32([]byte("nonexistent"))

	_, err := db.SilaPayloadEnvelopeByBlockHash(ctx, nonExistent)
	require.ErrorContains(t, "not found", err)
}

func TestStore_DeleteSilaPayloadEnvelope_CleansBlockHashIndex(t *testing.T) {
	db := setupDB(t)
	ctx := context.Background()
	env := testEnvelope(t)
	blockRoot := bytesutil.ToBytes32(env.Message.BeaconBlockRoot)
	...
	_, err := db.SilaPayloadEnvelopeByBlockHash(ctx, blockHash)
	require.NoError(t, err)
