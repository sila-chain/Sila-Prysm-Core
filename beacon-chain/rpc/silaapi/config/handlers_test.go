...
func TestGetSpec(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "http://example.com/sila/v1/config/spec", nil)
	writer := httptest.NewRecorder()
	writer.Body = &bytes.Buffer{}
...
func TestForkSchedule_Ok(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		request := httptest.NewRequest(http.MethodGet, "http://example.com/sila/v1/config/fork_schedule", nil)
		writer := httptest.NewRecorder()
		writer.Body = &bytes.Buffer{}
