...
import (
	...
	rpchelpers "github.com/sila-chain/Sila-Consensus-Core/v7/beacon-chain/rpc/silaapi/helpers"
...
func (s *Server) GetAggregateAttestationV2(w http.ResponseWriter, r *http.Request) {
	...
	if httputil.RespondWithSsz(r) {
		...
		if v >= version.Electra {
			typedAgg, ok := agg.(*silapbalpha.AttestationElectra)
			if !ok {
				httputil.HandleError(w, fmt.Sprintf("Attestation is not of type %T", &silapbalpha.AttestationElectra{}), http.StatusInternalServerError)
				return
			}
			...
			typedAgg, ok := agg.(*silapbalpha.Attestation)
			if !ok {
				httputil.HandleError(w, fmt.Sprintf("Attestation is not of type %T", &silapbalpha.Attestation{}), http.StatusInternalServerError)
				return
			}
			data, err = typedAgg.MarshalSSZ()
			if err != nil {
...
func (s *Server) aggregatedAttestation(w http.ResponseWriter, slot primitives.Slot, attDataRoot []byte, index primitives.CommitteeIndex) silapbalpha.Att {
	var match []silapbalpha.Att
	var err error
	...
	if len(match) > 0 {
		...
		slices.SortFunc(match, func(a, b silapbalpha.Att) int {
...
func matchingAtts(atts []silapbalpha.Att, slot primitives.Slot, attDataRoot []byte, index primitives.CommitteeIndex) ([]silapbalpha.Att, error) {
	if len(atts) == 0 {
		return []silapbalpha.Att{}, nil
	}

	postElectra := slots.ToForkVersion(slot) >= version.Electra
	result := make([]silapbalpha.Att, 0)
...
func (s *Server) SubmitSignedProposerPreferences(w http.ResponseWriter, r *http.Request) {
	...
	req := &silapbalpha.SubmitSignedProposerPreferencesRequest{
		SignedProposerPreferences: make([]*silapbalpha.SignedProposerPreferences, len(data)),
	}
...
func (s *Server) GetAttestationData(w http.ResponseWriter, r *http.Request) {
	...
	attestationData, rpcError := s.CoreService.GetAttestationData(ctx, &silapbalpha.AttestationDataRequest{
...
func (s *Server) produceSyncCommitteeContribution(
	...
	sig, aggregatedBits, err := s.CoreService.AggregatedSigAndAggregationBits(
		ctx,
		&silapbalpha.AggregatedSigAndAggregationBitsRequest{
			Msgs:      msgs,
			Slot:      slot,
			SubnetId:  index,
			BlockRoot: blockRoot,
...
func (s *Server) RegisterValidator(w http.ResponseWriter, r *http.Request) {
	...
	registrations := make([]*silapbalpha.SignedValidatorRegistrationV1, len(jsonRegistrations))
