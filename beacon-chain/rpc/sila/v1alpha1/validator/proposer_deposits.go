...
// block. Determining deposits depends on the current silaData vote for the block and whether or not
// this silaData has enough support to be considered for deposits inclusion. If current vote has
...
func (vs *Server) deposits(
	...
	canonicalSilaData, canonicalSilaDataHeight, err := vs.canonicalSilaData(ctx, beaconState, currentVote)
	if err != nil {
		return nil, err
	}
	...
	allPendingContainers := vs.PendingDepositsFetcher.PendingContainers(ctx, canonicalSilaDataHeight)
	if len(allPendingContainers) == 0 {
		log.Debug("No pending deposits for inclusion in block")
		return []*silapb.Deposit{}, nil
	}

	depositTrie, err := vs.depositTrie(ctx, canonicalSilaData, canonicalSilaDataHeight)
	if err != nil {
	...
	for _, dep := range allPendingContainers {
		if beaconState.Version() < version.Electra {
			// Add deposits up to min(MAX_DEPOSITS, sila_data.deposit_count - state.silaexec_deposit_index)
			if uint64(dep.Index) >= beaconState.SilaExecutionDepositIndex() && uint64(dep.Index) < canonicalSilaData.DepositCount {
			...
			silaExecutionDepositIndexLimit := min(canonicalSilaData.DepositCount, requestsStartIndex)
...
func (vs *Server) depositTrie(ctx context.Context, canonicalSilaData *silapb.SilaData, canonicalSilaDataHeight *big.Int) (cache.MerkleTree, error) {
	...
	upToSilaDataDeposits := vs.DepositFetcher.NonFinalizedDeposits(ctx, finalizedDeposits.MerkleTrieIndex(), canonicalSilaDataHeight)
	insertIndex := finalizedDeposits.MerkleTrieIndex() + 1

	if shouldRebuildTrie(canonicalSilaData.DepositCount, uint64(len(upToSilaDataDeposits))) {
		log.WithFields(logrus.Fields{
			"unfinalizedDeposits": len(upToSilaDataDeposits),
			"totalDepositCount":   canonicalSilaData.DepositCount,
		}).Warn("Too many unfinalized deposits, building a deposit trie from scratch.")
		return vs.rebuildDepositTrie(ctx, canonicalSilaData, canonicalSilaDataHeight)
	}
	for _, dep := range upToSilaDataDeposits {
	...
	valid, err := validateDepositTrie(depositTrie, canonicalSilaData)
...
func (vs *Server) rebuildDepositTrie(ctx context.Context, canonicalSilaData *silapb.SilaData, canonicalSilaDataHeight *big.Int) (cache.MerkleTree, error) {
	ctx, span := trace.StartSpan(ctx, "ProposerServer.rebuildDepositTrie")
	defer span.End()

	deposits := vs.DepositFetcher.AllDeposits(ctx, canonicalSilaDataHeight)
...
func validateDepositTrie(trie cache.MerkleTree, canonicalSilaData *silapb.SilaData) (bool, error) {
	if trie == nil || canonicalSilaData == nil {
		return false, errors.New("nil trie or silaData provided")
	}
	if trie.NumOfItems() != int(canonicalSilaData.DepositCount) {
		return false, errors.Errorf("wanted the canonical count of %d but received %d", canonicalSilaData.DepositCount, trie.NumOfItems())
	}
	rt, err := trie.HashTreeRoot()
	if err != nil {
		return false, err
	}
	if !bytes.Equal(rt[:], canonicalSilaData.DepositRoot) {
		return false, errors.Errorf("wanted the canonical deposit root of %#x but received %#x", canonicalSilaData.DepositRoot, rt)
	}
	return true, nil
}
...
func constructMerkleProof(trie cache.MerkleTree, index int, deposit *silapb.Deposit) (*silapb.Deposit, error) {
	...
	// in-memory deposits trie, which is updated only once the state's LatestSilaData
