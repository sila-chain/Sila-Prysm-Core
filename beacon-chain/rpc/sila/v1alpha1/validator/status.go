...
//
// Spec:
// https://github.com/sila-chain/Sila-Consensus-Specs/blob/master/sync/optimistic.md
...
func (vs *Server) validatorStatus(
	...
	case silapb.ValidatorStatus_UNKNOWN_STATUS:
		...
		dep, silaexecBlockNumBigInt := vs.DepositFetcher.DepositByPubkey(ctx, pubKey)
		if silaexecBlockNumBigInt == nil { // No deposit found in SILAEXEC.
			return resp, nonExistentIndex
		}
	...
	case silapb.ValidatorStatus_DEPOSITED, silapb.ValidatorStatus_PENDING, silapb.ValidatorStatus_PARTIALLY_DEPOSITED:
		if resp.Status == silapb.ValidatorStatus_PENDING {
			if vs.DepositFetcher == nil {
				log.Warn("Not connected to SILAEXEC. Cannot determine validator SILAEXEC deposit.")
			} else {
