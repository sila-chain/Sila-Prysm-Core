...
// This returns the local sila payload of a given slot. The function has full awareness of pre and post merge.
...
// This returns the local sila payload of a slot, proposer ID, and parent root assuming payload Is cached.
...
func (vs *Server) getLocalPayloadFromEngine(
	...
	// Blocks before Bellatrix don't have sila payloads. Use zeros as the hash.
...
func (vs *Server) getTerminalBlockHashIfExists(ctx context.Context, transitionTime uint64) ([]byte, bool, error) {
	...
	if params.BeaconConfig().TerminalBlockHash != params.BeaconConfig().ZeroHash {
		exists, _, err := vs.SilaExecutionBlockFetcher.BlockExists(ctx, terminalBlockHash)
		if err != nil {
...
func (vs *Server) applyParentSilaPayloadToHead(ctx context.Context, head state.BeaconState, parentRoot [32]byte) error {
	...
	envelope, err := vs.BeaconDB.SilaPayloadEnvelope(ctx, parentRoot)
	if err != nil {
...
//
// For states of version Capella or later, the block hash is directly retrieved from the state's latest sila payload header.
//
// If the merge transition has been completed, the parent block hash is also retrieved from the state's latest sila payload header.
//
...
func (vs *Server) getParentBlockHash(ctx context.Context, st state.BeaconState, slot primitives.Slot, headRoot [32]byte, parentFull bool) ([]byte, error) {
	if st.Version() >= version.Gloas {
		...
		bid, err := st.LatestSilaPayloadBid()
		if err != nil {
			return nil, errors.Wrap(err, "could not get latest sila payload bid")
		}
		if parentFull {
			bh := bid.BlockHash()
			return bh[:], nil
