...
func (vs *Server) GetBeaconBlock(ctx context.Context, req *silapb.BlockRequest) (*silapb.GenericBeaconBlock, error) {
...
func (vs *Server) BuildBlockParallel(ctx context.Context, sBlk interfaces.SignedBeaconBlock, head state.BeaconState, skipMevBoost bool, builderBoostFactor primitives.Gwei, parentFull bool) (*silapb.GenericBeaconBlock, error) {
	...
	var bundle silaenginev1.BlobsBundler
...
func (vs *Server) ProposeBeaconBlock(ctx context.Context, req *silapb.GenericSignedBeaconBlock) (*silapb.ProposeResponse, error) {
	var (
		blobSidecars       []*silapb.BlobSidecar
	...
	if block.IsBlinded() && slots.ToEpoch(block.Block().Slot()) >= params.BeaconConfig().FuluForkEpoch {
		...
		return &silapb.ProposeResponse{BlockRoot: root[:]}, nil
	}
...
func (vs *Server) handleBlindedBlock(ctx context.Context, block interfaces.SignedBeaconBlock) (interfaces.SignedBeaconBlock, []*silapb.BlobSidecar, error) {
...
) ([]*silapb.BlobSidecar, []blocks.RODataColumn, error) {
...
func (vs *Server) broadcastAndReceiveBlobs(ctx context.Context, sidecars []*silapb.BlobSidecar, root [fieldparams.RootLength]byte) error {
...
func (vs *Server) PrepareBeaconProposer(
	_ context.Context, request *silapb.PrepareBeaconProposerRequest,
) (*emptypb.Empty, error) {
...
func (vs *Server) GetFeeRecipientByPubKey(ctx context.Context, request *silapb.FeeRecipientByPubKeyRequest) (*silapb.FeeRecipientByPubKeyResponse, error) {
	...
	resp, err := vs.ValidatorIndex(ctx, &silapb.ValidatorIndexRequest{PublicKey: request.PublicKey})
	if err != nil {
		if strings.Contains(err.Error(), "Could not find validator index") {
			return &silapb.FeeRecipientByPubKeyResponse{
...
func (vs *Server) handlePostBlockStateError(ctx context.Context, block interfaces.SignedBeaconBlock, err error) (state.BeaconState, error) {
	...
	case errors.Is(err, transition.ErrProcessProposerSlashingsFailed):
		...
		block.SetProposerSlashings([]*silapb.ProposerSlashing{})
	case errors.Is(err, transition.ErrProcessAttesterSlashingsFailed):
		...
		if err := block.SetAttesterSlashings([]silapb.AttSlashing{}); err != nil {
	...
	case errors.Is(err, transition.ErrProcessVoluntaryExitsFailed):
		...
		block.SetVoluntaryExits([]*silapb.SignedVoluntaryExit{})
...
func (vs *Server) SubmitValidatorRegistrations(ctx context.Context, reg *silapb.SignedValidatorRegistrationsV1) (*emptypb.Empty, error) {
	if vs.BlockBuilder == nil || !vs.BlockBuilder.Configured() {
...
func blobsAndProofs(req *silapb.GenericSignedBeaconBlock) ([][]byte, [][]byte, error) {
	switch {
