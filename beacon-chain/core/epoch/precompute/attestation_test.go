...
func TestUpdateValidator_Works(t *testing.T) {
	...
	a := &silapb.PendingAttestation{InclusionDelay: 1, ProposerIndex: 2}
...
func TestSameHead(t *testing.T) {
	...
	same, err := precompute.SameHead(beaconState, &silapb.PendingAttestation{Data: att.Data})
...
func TestSameTarget(t *testing.T) {
	...
	same, err := precompute.SameTarget(beaconState, &silapb.PendingAttestation{Data: att.Data}, 0)
	require.NoError(t, err)
	assert.Equal(t, true, same, "Head in state does not match head in attestation")
	newRoot := [32]byte{'B'}
...
func TestAttestedPrevEpoch(t *testing.T) {
	...
	att.Data.Target.Root = r[:]
	att.Data.BeaconBlockRoot = r[:]
	votedEpoch, votedTarget, votedHead, err := precompute.AttestedPrevEpoch(beaconState, &silapb.PendingAttestation{Data: att.Data})
	require.NoError(t, err)
...
func TestAttestedCurrentEpoch(t *testing.T) {
	...
	att.Data.Target.Root = r[:]
	att.Data.BeaconBlockRoot = r[:]
	votedEpoch, votedTarget, err := precompute.AttestedCurrentEpoch(beaconState, &silapb.PendingAttestation{Data: att.Data})
	require.NoError(t, err)
	assert.Equal(t, true, votedEpoch, "Did not vote epoch")
	assert.Equal(t, true, votedTarget, "Did not vote target")
}
...
func TestProcessAttestations(t *testing.T) {
	...
	att1 := &silapb.Attestation{Data: &silapb.AttestationData{
		Target: &silapb.Checkpoint{Epoch: 0}},
		AggregationBits: bf}
	att2 := &silapb.Attestation{Data: &silapb.AttestationData{
		Target: &silapb.Checkpoint{Epoch: 0}},
		AggregationBits: bf}
	rt := [32]byte{'A'}
	...
	err := beaconState.AppendPreviousEpochAttestations(&silapb.PendingAttestation{Data: att1.Data, AggregationBits: bf, InclusionDelay: 1})
	require.NoError(t, err)
	err = beaconState.AppendCurrentEpochAttestations(&silapb.PendingAttestation{Data: att2.Data, AggregationBits: bf, InclusionDelay: 1})
	require.NoError(t, err)
