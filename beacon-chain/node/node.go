...
import (
	...
	"github.com/sila-chain/Sila-Consensus-Core/v7/beacon-chain/node/registration"
	...
	regularsync "github.com/sila-chain/Sila-Consensus-Core/v7/beacon-chain/sync"
	...
	"github.com/sila-chain/Sila-Consensus-Core/v7/beacon-chain/sync/checkpoint"
	initialsync "github.com/sila-chain/Sila-Consensus-Core/v7/beacon-chain/sync/initial-sync"
...
func configureBeacon(cliCtx *cli.Context) error {
	...
	if err := configureSilaExecutionConfig(cliCtx); err != nil {
		return errors.Wrap(err, "could not configure silaexec config")
	}
...
func startBaseServices(cliCtx *cli.Context, beacon *BeaconNode, depositAddress string, clearer *dbClearer) (*backfill.Store, error) {
	...
	if err := beacon.startStateGen(ctx, bfs, beacon.forkChoicer); err != nil {
		if errors.Is(err, stategen.ErrNoGenesisBlock) {
			log.Errorf("No genesis block/state is found. Sila only provides a mainnet genesis "+
...
func (b *BeaconNode) registerBlockchainService(fc forkchoice.ForkChoicer, gs *startup.ClockSynchronizer, syncComplete chan struct{}) error {
	...
	opts := append(
		...
		blockchain.WithSilaEngineCaller(web3Service),
