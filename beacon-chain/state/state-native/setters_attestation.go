...
func (b *BeaconState) RotateAttestations() error {
	...
	b.setCurrentEpochAttestations([]*silapb.PendingAttestation{})
	return nil
}

func (b *BeaconState) setPreviousEpochAttestations(val []*silapb.PendingAttestation) {
...
func (b *BeaconState) setCurrentEpochAttestations(val []*silapb.PendingAttestation) {
...
// to the end of list.
func (b *BeaconState) AppendCurrentEpochAttestations(val *silapb.PendingAttestation) error {
	b.lock.Lock()
	defer b.lock.Unlock()
	...
	if b.sharedFieldReferences[types.CurrentEpochAttestations].Refs() > 1 {
		...
		atts = make([]*silapb.PendingAttestation, len(b.currentEpochAttestations))
...
// to the end of list.
func (b *BeaconState) AppendPreviousEpochAttestations(val *silapb.PendingAttestation) error {
	b.lock.Lock()
	defer b.lock.Unlock()
	...
	if b.sharedFieldReferences[types.PreviousEpochAttestations].Refs() > 1 {
		atts = make([]*silapb.PendingAttestation, 0, len(b.previousEpochAttestations)+1)
...
func (b *BeaconState) SetPreviousEpochAttestations(a []*silapb.PendingAttestation) error {
	b.lock.Lock()
	defer b.lock.Unlock()
...
func (b *BeaconState) SetCurrentEpochAttestations(a []*silapb.PendingAttestation) error {
	b.lock.Lock()
	defer b.lock.Unlock()
