...
// to and from its own address) to advance the chain until it has moved forward SilaExecution_FOLLOW_DISTANCE blocks.
func (d *Depositor) SendAndMine(ctx context.Context, offset, nvals int, batch types.DepositBatch, partial bool) error {
	...
	if err = WaitForBlocks(ctx, d.Client, d.Key, params.BeaconConfig().SilaExecutionFollowDistance); err != nil {
