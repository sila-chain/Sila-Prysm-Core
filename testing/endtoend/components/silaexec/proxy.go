...
func (node *Proxy) Start(ctx context.Context) error {
	f, err := os.Create(path.Join(e2e.TestParams.LogPath, "silaexec_proxy_"+strconv.Itoa(node.index)+".log"))
	if err != nil {
		return err
	}
	jwtPath := path.Join(e2e.TestParams.TestPath, "silaData/"+strconv.Itoa(node.index)+"/")
	if node.index == 0 {
	...
	opts := []proxy.Option{
		proxy.WithDestinationAddress(fmt.Sprintf("http://127.0.0.1:%d", e2e.TestParams.Ports.SilaExecutionAuthRPCPort+node.index)),
		proxy.WithPort(e2e.TestParams.Ports.SilaExecutionProxyPort + node.index),
	...
	log.Infof("Starting silaexec proxy %d with port: %d and file %s", node.index, e2e.TestParams.Ports.SilaExecutionProxyPort+node.index, f.Name())
...
// Started checks whether the silaexec proxy is started and ready to be queried.
