...
func RunForkTransitionTest(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "gloas", "transition/core/pyspec_tests")
	for _, folder := range testFolders {
		t.Run(folder.Name(), func(t *testing.T) {
			...
			bc.BlobSchedule = []params.BlobScheduleEntry{{MaxBlobsPerBlock: 9}}
			bc.FuluForkEpoch = 0
			bc.GloasForkEpoch = types.Epoch(config.ForkEpoch)
