...
func RunSilaDataResetTests(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "bellatrix", "epoch_processing/sila_data_reset/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "bellatrix", "epoch_processing/sila_data_reset/pyspec_tests")
	}
...
func processSilaDataResetWrapper(t *testing.T, st state.BeaconState) (state.BeaconState, error) {
	st, err := epoch.ProcessSilaDataReset(st)
