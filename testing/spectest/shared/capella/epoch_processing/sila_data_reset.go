...
func RunSilaDataResetTests(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "capella", "epoch_processing/sila_data_reset/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "capella", "epoch_processing/sila_data_reset/pyspec_tests")
	}
