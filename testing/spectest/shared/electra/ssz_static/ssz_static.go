...
func customHtr(t *testing.T, htrs []common.HTR, object any) []common.HTR {
	_, ok := object.(*silapb.BeaconStateElectra)
	if !ok {
		return htrs
	}

	htrs = append(htrs, func(s any) ([32]byte, error) {
		beaconState, err := state_native.InitializeFromProtoUnsafeElectra(s.(*silapb.BeaconStateElectra))
