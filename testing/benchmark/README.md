# Benchmarks for Sila-Prysm State Transition
This package contains the functionality needed for benchmarking Sila-Prysm state transitions, this includes its block processing (with and without caching) and epoch processing functions. There is also a benchmark for HashTreeRoot on a large beacon state.
...
To generate new files to use for benchmarking, run the below command in the root of Sila-Prysm.
