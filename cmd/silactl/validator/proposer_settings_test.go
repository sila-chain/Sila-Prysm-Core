...
func getValidatorHappyPathTestServer(t *testing.T) *httptest.Server {
	...
	return httptest.NewUnstartedServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet {
			if r.RequestURI == "/sila/v1/keystores" {
			...
			} else if r.RequestURI == "/sila/v1/remotekeys" {
...
func TestValidateValidateIsSilaAddress(t *testing.T) {
	t.Run("Happy Path", func(t *testing.T) {
		err := validateIsSilaAddress("0xb698D697092822185bF0311052215d5B5e1F3933")
		require.NoError(t, err)
	})
	t.Run("Too Long", func(t *testing.T) {
		err := validateIsSilaAddress("0xb698D697092822185bF0311052215d5B5e1F39331")
		require.ErrorContains(t, "no default address entered", err)
	})
	t.Run("Too Short", func(t *testing.T) {
		err := validateIsSilaAddress("0xb698D697092822185bF0311052215d5B5e1F393")
		require.ErrorContains(t, "no default address entered", err)
	})
	t.Run("Not a hex", func(t *testing.T) {
		err := validateIsSilaAddress("b698D697092822185bF0311052215d5B5e1F393310")
		require.ErrorContains(t, "no default address entered", err)
	})
}
